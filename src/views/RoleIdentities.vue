  ...
  identities.value.forEach((identity) => {
    result.push(
      reactive({
        ...
        active: (role.value!.identities ?? []).some(
...
const refresh = async () => {
  ...
  const identityResponse = await accessApi.post<
    IdentitySpecification,
    AxiosResponse<Identity[]>
...
const toggle = async (item: IdentityItem) => {
  ...
  await accessApi.patch(`v1/identities/${item.id}/roles/${props.id}/status`, {
    active: item.active,
  });
