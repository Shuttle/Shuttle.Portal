  ...
  <v-card flat>
    ...
    >
      ...
      <template v-slot:item.permission="{ item }">
        <span :class="!item.active ? 'text-gray-500' : ''">{{
          item.name
        }}</span>
      </template>
    </s-data-table>
...
import type { IdentifierAvailability, Permission } from "@/portal";
...
const refresh = async () => {
  const roleResponse = await accessApi.get(`v1/roles/${props.id}`);

  name.value = roleResponse.data.name;
  rolePermissions.value = roleResponse.data.permissions;

  const permissionsResponse = await accessApi.post("v1/permissions/search", {});
...
const getPermissionAvailability = async () => {
  ...
  const response = await accessApi.post<IdentifierAvailability[]>(
    `v1/roles/${props.id}/permissions/availability`,
    {
  ...
  response.data.forEach((availability) => {
    ...
    permissionItem.working = permissionItem.activeOnToggle
      ? availability.active
      : !availability.active;
  });
...
const toggle = async (item: PermissionItem) => {
  ...
  await accessApi.patch(`v1/roles/${props.id}/permissions/${item.id}/status`, {
    active: item.active,
  });
