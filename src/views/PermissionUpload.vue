<template>
  <s-form :submit="submit">
    <s-title :title="$t('permission-upload')" close-drawer type="borderless" />
    <v-file-upload
      density="comfortable"
      v-model="state.file"
      :icon="`svg:${mdiCloudUploadOutline}`"
      :title="$t('drag-and-drop-file')"
      :multiple="false"
    ></v-file-upload>
    ...
    <v-alert
      ...
      variant="outlined"
      type="error"
      v-if="validation.message('file')"
    >
      {{ validation.message("file") }}
    </v-alert>
    <s-strip reverse class="mt-2">
...
const submit = async () => {
  ...
  try {
    ...
    await accessApi.post("v1/permissions/file", formData, {
      headers: {
