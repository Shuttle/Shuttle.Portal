...
const refresh = async () => {
  ...
  try {
    const tenantResponse = await accessApi.post("v1/tenants/search", {});

    tenants.value = tenantResponse.data;

    const identityResponse = await accessApi.get(`v1/identities/${id.value}`);
...
const getWorkingTenants = async () => {
  ...
  const response = await accessApi.post(
    `v1/identities/${id.value}/tenants/availability`,
    {
  ...
  response.data.forEach((availability: IdentifierAvailability) => {
    ...
    tenantItem.working = tenantItem.activeOnToggle
      ? availability.active
      : !availability.active;
  });
...
const toggle = async (item: TenantItem) => {
  ...
  await accessApi.patch(
    `v1/identities/${id.value}/tenants/${item.tenantId}/status`,
    {
      active: item.active,
    },
  );
