<template>
  <s-form :submit="change" :size="getSize()">
    <s-title
      :title="$t('change-password')"
      :close-path="getClosePath()"
      :type="getTitleType()"
    />
    <v-text-field
...
import type { ChangePassword } from "@/portal";
...
const getSize = (): "small" | "full" => {
  return props.id === "token" ? "small" : "full";
};
...
const change = async () => {
  ...
  try {
    await accessApi.patch<ChangePassword>("v1/identities/password", {
...
onMounted(() => {
  ...
  accessApi.get(`v1/identities/${props.id}`).then((response) => {
