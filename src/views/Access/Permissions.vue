...
import type { Permission } from "@/portal";
...
const getWorkingPermissions = async () => {
  ...
  try {
    const response = await accessApi.post(`v1/permissions/search`, {
...
const setStatus = async (permission: PermissionItem) => {
  ...
  try {
    await accessApi.patch(`v1/permissions/${permission.id}`, {
